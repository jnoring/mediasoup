...
class Router extends EnhancedEventEmitter
{
   ...
   *
	...
	/**
	 * Create a AudioLevelObserver.
	 *
	 * @param {Number} [threshold=-80] - Minimum volume (in dBvo from -127 to 0) to
	 *                                   be considered.
	 ...
	 */
	async createAudioLevelObserver({ threshold = -80, interval = 1000 } = {})
	{
		logger.debug('createAudioLevelObserver()');

		const internal = { ...this._internal, rtpObserverId: uuidv4() };
		const reqData = { threshold, interval };
