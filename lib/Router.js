...
class Router extends EnhancedEventEmitter
{
	 ...
	 * @emits {transport: Transport} observer:newtransport
	 * @emits @close
	 */
   ...
   *
	...
	)
	{
		...
		// Emit observer event.
		this.safeEmit('observer:newtransport', transport);
   ...
   * @private
	 ...
	 *
	 * @async
	 * @returns {Object} - Includes toRouter() method.
	 */
	async pipe({ producerId } = {})
	{
		if (!producerId)
		...
		return {
			 ...
			 *
			 * @param {Router} router
			 * @param {String|Object} listenIp - Listen IP string or an object with ip
			 *   and optional announcedIp string.
			 *
			 * @async
			 * @returns {Producer} - Producer created in the given Router.
			 */
			toRouter : async ({ router, listenIp }) =>
			{
				if (!router)
					throw new TypeError('Router not found');

				const pipeTransports = this._mapRouterPipeTransports.get(router);
				...
				else
				{
					try
					{
						localPipeTransport = await this.createPipeTransport({ listenIp });
						remotePipeTransport = await router.createPipeTransport({ listenIp });

						await localPipeTransport.connect(
							{
								ip   : remotePipeTransport.tuple.ip,
								port : remotePipeTransport.tuple.port
							});

						await remotePipeTransport.connect(
							{
								ip   : localPipeTransport.tuple.ip,
								port : localPipeTransport.tuple.port
							});

						localPipeTransport.on('observer:close', () =>
						{
							remotePipeTransport.close();
							this._mapRouterPipeTransports.delete(router);
						});

						remotePipeTransport.on('observer:close', () =>
						{
					...
					{
						logger.error(
							'pipe().toRouter() | error creating PipeTransport pair:%o',
							error);
					...
					{
						logger.error(
							'pipe().toRouter() | error creating Consumer/Producer pair:%o',
							error);
