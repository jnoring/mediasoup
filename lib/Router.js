   ...
   * @private
	...
	async pipe({ producerId } = {})
	{
		...
		return {
			 ...
			 *
			 * @async
			 * @returns {Object} - Contains `pipeProducer` {Producer} created in the given
			 *   destination Router and the `pipeConsumer` {Consumer} created in current
			 *   Router.
			 */
			toRouter : async ({ router, listenIp = '127.0.0.1' } = {}) =>
			{
				if (!router)
				...
				try
				{
					...
					// Close the pipe Producer when the pipe Consumer is closed.
					pipeConsumer.on('observer:close', () => pipeProducer.close());
				}
				catch (error)
				{
					logger.error(
