...
class Router extends EnhancedEventEmitter
{
   ...
   *
	 ...
	 *   auto-detected based on first RTP/RTCP packet received.
	 * @param {Object} [appData={}] - Custom app data.
   *
	 * @async
	...
	)
	{
		...
		const internal = { ...this._internal, transportId: uuidv4() };
		const reqData = { listenIp, rtcpMux, comedia };
   ...
   *
	...
	async pipeToRouter({ producerId, router, listenIp = '127.0.0.1' } = {})
	{
		...
		const pipeTransportPair = this._mapRouterPipeTransports.get(router);
		...
		try
		{
			...
			pipeConsumer.on('observer:pause', () => pipeProducer.pause());
			pipeConsumer.on('observer:resume', () => pipeProducer.resume());

			// Pipe events from the pipe Producer to the pipe Consumer.
			pipeProducer.on('observer:close', () => pipeConsumer.close());
