...
class Transport extends EnhancedEventEmitter
{
   ...
   *
	...
	async produce({ kind, rtpParameters, appData = {} } = {})
	{
		...
		// If no CNAME is given, create a random one since it will copied verbatim
		// to Consumers and the Consumer will use that value anyway in its RTCP SDES
		// chunks.
		if (!rtpParameters.rtcp || !rtpParameters.rtcp.cname)
		{
			rtpParameters.rtcp = rtpParameters.rtcp || {};
			rtpParameters.rtcp.cname = uuidv4().substr(0,8);
		}
